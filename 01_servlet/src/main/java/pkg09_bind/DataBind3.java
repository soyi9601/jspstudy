...
public class DataBind3 extends HttpServlet {
    ...
    // response.sendRedirect로 안되는 이유는 : redirect는 2개의 요청으로 이루어져있음. redirect 보내면서 1개의 요청이 끝났기 때문에 redirect로 쓰게 되면 null 값이 나옴.
    request.getRequestDispatcher("/dataConfirm").forward(request, response);
    
    // 일회용 저장소 : HttpServletRequest
    // 다회용 저장소 : 1. Session(서버 측 공간 -> 보안이 좀 더 월등함: 로그인), 2. Cookie (클라이언트 측 공간 -> 보안에 취약함)
